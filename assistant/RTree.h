...
class ELEMTYPEREAL = ELEMTYPE, int TMAXNODES = 4, int TMINNODES = TMAXNODES / 2>
class RTree
{
protected:
	struct Node;
...
public:
	...
	///draw the tree rect to pic for check is right of every step
	void drawbyopencv(char* path);
	///draw the tree rect to pic for check is right of every step
	void DrawRect(IplImage*,Node* path);
	...
	//测试多边形rTree布尔运算
	//传给线程的结构体,包含了树的指针还有当前ROOT下面的index
	...
	Paths resPaths;
#endif
	CCriticalSection m_g_cs;

	//单结点计算线程
	static UINT ThreadCalNode(LPVOID lParam)
	{
		calnodstruct *pcon = (calnodstruct *)lParam;
		pcon->tmp = pcon->ptree->GetFinalPolgon_Clipper(pcon->ptree->m_root->m_branch[pcon->index].m_child);
		pcon->ptree->m_g_cs.Lock();
		if (pcon->ptree->resPaths.size() == 0)
		{
			pcon->ptree->resPaths = pcon->tmp;
		}
		else{;
#ifdef USE_CLIPPER
			Clipper c;
			c.AddPaths(pcon->ptree->resPaths, ptSubject, true);
			c.AddPaths(pcon->tmp, ptClip, true);
			c.Execute(ClipType::ctUnion, pcon->ptree->resPaths, pftEvenOdd, pftEvenOdd);
			MyBoolClass::sortZeroIndex(pcon->ptree->resPaths,theApp.m_mainFrame->m_GerberData.m_cgal.m_clipper.Record_Circle);
#endif
		}
		pcon->ptree->m_g_cs.Unlock();
		if(*pcon->handle == NULL)
			WaitForSingleObject(NULL,10);
		*pcon->handle = NULL;
		//pcon->tmp.clear();
...
#ifdef USE_CGAL
	Polygon_set_2 calBoolean(){return GetFinalPolgon(m_root);}
#endif
#ifdef USE_CLIPPER
	void calBoolean_clipper(){
		if(m_root->m_level < 2){
			resPaths = GetFinalPolgon_Clipper(m_root);
			return;
		}
		resPaths.resize(0);
		CWinThread* pA = NULL;
		HANDLE handle[128] = {0};
		calnodstruct* groupNodeData = new calnodstruct[m_root->m_count];
		for (int i = 0; i < m_root->m_count; i++)
		{
			calnodstruct *tmp = &groupNodeData[i];
			tmp->handle = &handle[i];
			handle[i] = NULL;
			tmp->ptree = this;
			tmp->index = i;
			pA = AfxBeginThread(ThreadCalNode,tmp);
			handle[i] = pA->m_hThread;
			Sleep(1);
		}
		for (int j = 0; j < m_root->m_count; j++){
			while(WaitForSingleObject(NULL,10)){
				if(handle[j] == NULL)
					break;
			}
		}
//		WaitForMultipleObjects(m_root->m_count,handle,TRUE, INFINITE);
//		delete[] groupNodeData;
		//根结点有多少个结点就开多少个线程
		//等待所有线程完成
	}
#endif
...
#ifdef USE_CGAL
	Polygon_set_2 GetFinalPolgon(Node* treenode)
	{
		Polygon_set_2 tmp;
		if (treenode->m_level == 0)
		{

			for (int indexOfsamelevel = 0; indexOfsamelevel < treenode->m_count; indexOfsamelevel++)
			{
				tmp.join(treenode->m_branch[indexOfsamelevel].m_data->m_png);
			}
		}
		else if(treenode->m_level > 0){
			for (int indexOfsamelevel = 0; indexOfsamelevel < treenode->m_count; indexOfsamelevel++)
			{
				tmp.join(GetFinalPolgon(treenode->m_branch[indexOfsamelevel].m_child));
			}
		}
		return tmp;
	}
...
#ifdef USE_CLIPPER
	Paths GetFinalPolgon_Clipper(Node* treenode)
	{
		Paths tmp;
		if (treenode->m_level == 0)
		{
			...
			for (int indexOfsamelevel = 0; indexOfsamelevel < treenode->m_count; indexOfsamelevel++)
			{
				...
				c.AddPaths(treenode->m_branch[indexOfsamelevel].m_data->m_png, ptClip, true);
				c.Execute(ClipType::ctUnion, tmp, pftEvenOdd, pftEvenOdd);
			}
		}
		else if(treenode->m_level > 0){
			for (int indexOfsamelevel = 0; indexOfsamelevel < treenode->m_count; indexOfsamelevel++)
			{
				Clipper c;
				c.AddPaths(tmp, ptSubject, true);
				c.AddPaths(GetFinalPolgon_Clipper(treenode->m_branch[indexOfsamelevel].m_child), ptClip, true);
				c.Execute(ClipType::ctUnion, tmp, pftEvenOdd, pftEvenOdd);
			}
		}
		MyBoolClass::sortZeroIndex(tmp,theApp.m_mainFrame->m_GerberData.m_cgal.m_clipper.Record_Circle);
		return tmp;
	}
#endif
...
RTREE_TEMPLATE
	void RTREE_QUAL::drawbyopencv(char* path)
{
	assert(m_root);
	Rect rect;
	for(int index = 0; index < NUMDIMS; ++index)
	...
	for(int i = 0 ; i < m_root->m_count;i++)
		rect = CombineRect(&rect,&m_root->m_branch[i].m_rect);

	IplImage* pImg = cvCreateImage(cvSize(rect.m_max[0] - rect.m_min[0],rect.m_max[1] - rect.m_min[1]),8,3);
	cvZero(pImg);
	cvNot(pImg,pImg);
	DrawRect(pImg,m_root);
	...
	ELEMTYPEREAL RTREE_QUAL::RectSphericalVolume(Rect* a_rect)
{
	...
	radius = (ELEMTYPEREAL)sqrt((float)sumOfSquares);
	...
	{
		return (ELEMTYPEREAL)(pow((float)radius, NUMDIMS) * m_unitSphereVolume);
	}
}
...
RTREE_TEMPLATE
	void RTREE_QUAL::DrawRect(IplImage* pImg,Node* node)
{
	if(pImg==NULL||node == NULL)
		return;
	CvScalar color;
	...
	for(int i = 0 ; i < node->m_count;i++)
		cvRectangle(pImg,cvPoint(node->m_branch[i].m_rect.m_min[0],node->m_branch[i].m_rect.m_max[1]),cvPoint(node->m_branch[i].m_rect.m_max[0],node->m_branch[i].m_rect.m_min[1]),color,1);

	if (node->m_level == 0)
		return;
	for(int i = 0 ; i < node->m_count;i++)
	{
		Node *cur = node->m_branch[i].m_child;
		DrawRect(pImg,cur);
	}
}
